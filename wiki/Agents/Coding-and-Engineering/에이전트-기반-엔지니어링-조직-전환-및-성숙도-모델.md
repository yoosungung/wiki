...
updated: "2026-08-31"
related_raw: ["[[raw/2026-08-31-goose-agent-framework-engineering-ai-maturity-model.md]]", "[[raw/2026-08-28-autonomous-mas-openclaw-webmcp-goose.md]]", "[[2026-07-16-agentic-engineering-organizations-block-case-study.md]]"]
...
| :--- | :--- | :--- | :--- | :--- |
| **Level 0** | **No AI** | 수동 코딩 | 개발자 (Developer) | AI 도구 미도입 상태 |
| **Level 1** | **Tab Complete** | 단순 라인/블록 단위 자동완성 | 개발자 (컨텍스트 제공) | 코드 추천 수용률 및 기본 코딩 보조 |
| **Level 2** | **Chat & Debug** | IDE 내 대화형 질문 및 해결 | 탐색자 (검증 및 대입) | 오류 복사-붙여넣기 및 단일 스크립트 작성 |
| **Level 3** | **Context Engineering** | **규칙 기반 맥락 주입 및 위임** | **설계자 / 가이드 작성자** | **[핵심 변곡점]** `AGENTS.md` 및 `.agents/rules`로 에이전트 동작 맥락 정의 |
| **Level 4** | **Skill Integration** | MCP 연결 및 외부 도구(API) 사용 | 감독관 | 파일 시스템, 터미널, Git 연동 등 실질적 도구 호출 제어 |
| **Level 5** | **Feedback Loops** | 자동 테스트 및 자가 디버깅(Self-Correction) | 피드백 제공자 | 오류 코드 파싱 및 테스트 패스 기반의 빌드 루프 자동 수정 |
| **Level 6** | **Background Agents** | 비동기 백그라운드 태스크 수행 | 승인권자 (Approver) | 장시간 수행(Overnight) 작업 및 에디터 밖 백그라운드 릴리즈 |
| **Level 7** | **Harness Engineering** | 가상 샌드박스 및 에이전트 환경 설계 | 샌드박스 보안 설계자 | 자율 실행에 따르는 격리망(Isolation), 보안 규칙 수립 |
| **Level 8** | **Autonomous Swarm** | 다중 에이전트 자율 스웜 협업 | 거버너 (Governor) | **[최종 진화]** 중앙 오케스트레이션 없는 독립 에이전트 간 분산 협력 및 완전 자율 PR 배포 |
