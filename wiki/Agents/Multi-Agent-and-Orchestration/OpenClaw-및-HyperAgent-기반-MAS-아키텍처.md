...
- **Sandbox multi-backend & Read-Only governance (2026-09-03)**: `sandbox.mode`(off/non-main/all), Docker/Podman/SSH/OpenShell 백엔드, `~/.openclaw` Read-Only 마운트를 통한 런타임 변조 차단, `openclaw sandbox list|recreate|explain`. 상세는 [[wiki/Agents/Multi-Agent-and-Orchestration/자율수행-멀티-에이전트-시스템-오케스트레이션-및-보안-격리-2026.md]] §2.16.
- **Secret egress / SQLite backup / plugin provenance (2026-08-16)**: v2026.8.1 secret→HTTPS host fail-closed, `openclaw backup sqlite …`, untrusted plugin `--force`, shared channel ingress monitor, `OPENCLAW_SUPERVISOR_MODE=external`. 상세는 [[wiki/Agents/Multi-Agent-and-Orchestration/자율수행-멀티-에이전트-시스템-오케스트레이션-및-보안-격리-2026.md]].
- **Managed worktrees / cleanup limits (2026-08-10)**: snapshot-before-delete, 7d idle GC, opt-in `maxCount`/`maxTotalSizeGb`, `sandbox recreate`. 상세는 [[wiki/Agents/Multi-Agent-and-Orchestration/자율수행-멀티-에이전트-시스템-오케스트레이션-및-보안-격리-2026.md]].
- **Dynamic model `/tools` inventory (2026-08-04)**: `/tools`가 prepared runtime model context publish/borrow **전**에 resolve해 임시 inventory failure — [#119306](https://github.com/openclaw/openclaw/pull/119306). 상세는 [[wiki/Agents/Multi-Agent-and-Orchestration/자율수행-멀티-에이전트-시스템-오케스트레이션-및-보안-격리-2026.md]].
- **Steering / auth cooldown (2026-08-05)**: active run steering(#119594), credential-only key cooldown(#119147). 상세 동일.
- **Gateway health auth / MMS TimeoutError (2026-08-03)**: `openclaw gateway health --token`(#118996), MMS stall→`TimeoutError` 재시도(#119002). 상세는 [[wiki/Agents/Multi-Agent-and-Orchestration/자율수행-멀티-에이전트-시스템-오케스트레이션-및-보안-격리-2026.md]].
- **Cron announce / Codex auth / Telegram preamble (2026-08-02)**: 멀티 채널 isolated announce 생성 게이트(#118272), Codex 마이그레이션 시 선택 OAuth 유지(#118205), Telegram CLI tool 중 pre-tool 텍스트 보존. 상세는 [[wiki/Agents/Multi-Agent-and-Orchestration/자율수행-멀티-에이전트-시스템-오케스트레이션-및-보안-격리-2026.md]].
- **Session memory flush / gateway list (2026-07-30)**: 부모 recovery와 memory flush 생명주기 분리(#116198), retained subagent history 하에서도 `sessions_list` 응답성(#116533), startup stall·WS heartbeat. 상세는 [[wiki/Agents/Multi-Agent-and-Orchestration/자율수행-멀티-에이전트-시스템-오케스트레이션-및-보안-격리-2026.md]].
...
- **Watched-session / sandbox browser (2026-07-28)**: ambient watched group/topic을 모델에 명시(#114835), Docker `network=none` 브라우저 사이드카 거부·`doctor --fix`(#115250), warm turn·session 바인딩. 상세는 [[wiki/Agents/Multi-Agent-and-Orchestration/자율수행-멀티-에이전트-시스템-오케스트레이션-및-보안-격리-2026.md]].
...
- **Control UI / discovery / restart (2026-07-26)**: path 기반 세션·대시보드 URL(`/chat/...`, `/dashboard/...`, session key 고정), Anthropic OAuth discovery(`sk-ant-oat` → Bearer), 재시작 시 `stopReason: aborted` 재개, ClawHub skill 아이콘. 상세는 [[wiki/Agents/Multi-Agent-and-Orchestration/자율수행-멀티-에이전트-시스템-오케스트레이션-및-보안-격리-2026.md]].
...
- **YOLO Mode vs. Auto Mode**:
    ...
    - **Auto Mode (2026-08-27 공식 블로그 보강)**:
        ...
        - **`tools.exec.mode`**: `deny|allowlist|ask|auto|full` — 상세 [[wiki/Agents/Multi-Agent-and-Orchestration/자율수행-멀티-에이전트-시스템-오케스트레이션-및-보안-격리-2026.md]] §2.15.
