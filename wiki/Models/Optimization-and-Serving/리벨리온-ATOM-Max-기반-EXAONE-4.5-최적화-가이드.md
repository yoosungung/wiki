...
- **vLLM-RBLN (v0.8.1+)**: vLLM v0.8.1부터 공식 플러그인 시스템으로 전환되었습니다. `pip install vllm-rbln`을 통해 핵심 엔진과 최적화 커널이 자동 통합되며, **PagedAttention** 및 **FlashAttention**이 리벨리온 NPU 아키텍처에 맞게 완전히 리엔지니어링되어 제공됩니다. 이를 통해 GPU 대비 훨씬 저렴한 TCO(소모 전력 대비 토큰 처리량)로 NPU-native 성능을 발휘합니다. `vllm serve` 가동 시 EXAONE 4.5의 자체 MTP(Multi-Token Prediction) speculative decoding 가속 설정을 `speculative_config` 파라미터를 통해 직접 연동 및 기동할 수 있습니다. (2026-08-27 업데이트: 2026년 리벨리온에 인수된 AI 추론 압축 전문 스타트업 SqueezeBits의 기술 노하우가 `optimum-rbln` 및 `vllm-rbln` 커널 최적화에 깊이 통합되어, 메모리 전송 병목을 최소화하는 하드웨어 친화적 양자화 및 서빙이 가능해졌습니다)
