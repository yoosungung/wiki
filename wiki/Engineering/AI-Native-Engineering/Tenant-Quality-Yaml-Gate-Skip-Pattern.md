...
| 게이트 | 필수 키(예) | 키 없음 |
| :--- | :--- | :--- |
| clean_code | `clean_code.command` | skip + 사유 기록, **NF 없음** |
| bulk_api | `bulk_api` (endpoints/command) | skip + 사유, NF 없음 |
| opik | `opik:` (project_name/dataset/command) | skip; fail/regression만 NF |
| load | `load.command` (test env) | skip + 사유; **실패만** NF |
...
5. MCP discovery가 깨져도 Active 스케줄 티켓은 JSON-RPC fallback(`getTicket`/`addComment`)으로 마감 가능 — [[wiki/Engineering/Infrastructure-and-DevOps/path-graph-Argo-ImagePullBackOff-runbook.md]] Closeout pitfall.
